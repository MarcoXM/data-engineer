...
def create_database():
    """Connect to Postgresql, drop any existing db and create a new one.
    Keyword arguments:
    * None
    Output:
    ...
    * cur --    cursory to connected DB. Allows to execute SQL commands.
    ...
    conn = psycopg2.connect("host=127.0.0.1 dbname=marcodb user=sqlmarco password=4mysiri")
    conn.set_session(autocommit=True)
    cur = conn.cursor()
    
    # create sparkify database with UTF8 encoding
    cur.execute("DROP DATABASE IF EXISTS spartifydb")
    cur.execute("CREATE DATABASE spartifydb WITH ENCODING 'utf8' TEMPLATE template0")
